make_score_row <- function(cluster, neuron_type, score, n_genes_used = 10,
                            n_genes_requested = 10, n_genes_missing = 0, n_genes_uninformative = 0) {
  tibble::tibble(
    cluster = cluster, neuron_type = neuron_type, score = score,
...
test_that("a clear winner (high score, large gap) is annotated", {
  ...
  expect_equal(result$second_neuron_type, "B")
...
test_that("a single-candidate matrix (no second neuron type) never auto-annotates", {
  m <- make_score_row("c1", "A", 0.95)
  result <- classify_cengen_matches(m, cutoff = 0.6, min_gap = 0.10)
  expect_true(is.na(result$second_neuron_type))
...
test_that("classify_cengen_matches errors informatively on a malformed score matrix", {
  expect_error(
    classify_cengen_matches(tibble::tibble(cluster = "c1", neuron_type = "A")),
