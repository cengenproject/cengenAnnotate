---
title: "Introduction to cengenAnnotate"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to cengenAnnotate}
...
usually means: run `Seurat::FindAllMarkers()`, copy the top ~20 marker
genes for a cluster, paste them into the CeNGEN website, and visually
judge whether the resulting dot plot shows one neuron type standing out.
`cengenAnnotate` computes that judgment directly from CeNGEN's reference
expression data, so most clusters can be annotated automatically and only
the genuinely ambiguous ones come back for manual review.
...
`prepare_marker_panel()` takes raw `Seurat::FindAllMarkers()` output and
reduces it to the top marker genes per cluster (default 20; equally
weighted, fold-change is only used to pick which genes make the panel).
...
`score_cengen_clusters()` scores every cluster against every candidate
neuron type and classifies each as `"annotate"` or in need of review.
...
specific across the marker panel; clusters below the score/gap cutoffs, or
with too few usable marker genes, come back as `"review_ambiguous"` /
...
`write_cengen_annotations()` is an explicit, opt-in step — clusters sent
to review are never silently annotated.

```{r, eval = requireNamespace("Seurat", quietly = TRUE) && requireNamespace("SeuratObject", quietly = TRUE)}
counts <- matrix(
  rpois(6 * 30, lambda = 3),
  nrow = 6, ncol = 30,
...
clusters), `plot_cengen_dotplot()` reproduces the CeNGEN-style view for one
cluster's marker panel using the same size/color signals the score is
built from:
...
`score_cengen_clusters()` for unattended annotation, calibrate them
against clusters with independently known identity and adjust as needed.
